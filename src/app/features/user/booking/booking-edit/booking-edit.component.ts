...
export class BookingEditComponent implements OnInit {
  ...
  get dayValue() {
    return moment(this.editBookForm.value.date).format('YYYY-MM-DD')
  }
  constructor(
  ...
  ngOnInit() {
    ...
    this.activeRoute.params.subscribe(linkRoute => {
      this.bookingService.getBookingById(linkRoute.id).subscribe(bookedData => {
        ...
        this.bookingService.getBookedHoursByDate(this.bookedObject.date,this.bookedObject.playground_id).subscribe((resultData)=>{
  ...
  onSubmit(){
    ...
    this.editBookForm.value.date= this.dayValue
